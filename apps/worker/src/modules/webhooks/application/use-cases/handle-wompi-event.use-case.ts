...
export class HandleWompiEventUseCase {
  ...
  async execute(
    ...
    if (existing) {
      await this.waitForIdempotencyTerminal(idempotencyKey);
      this.logger.log("webhook_accepted", {
  ...
  private async waitForIdempotencyTerminal(key: string): Promise<void> {
    for (let attempt = 0; attempt < this.wait.attempts; attempt += 1) {
      const record = await this.idempotency.find(key);
      if (!record || record.response || record.errorCode) {
        return;
      }
