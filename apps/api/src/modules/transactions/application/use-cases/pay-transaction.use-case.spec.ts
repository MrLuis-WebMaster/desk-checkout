...
describe("PayTransactionUseCase", () => {
  ...
  beforeEach(() => {
    ...
    writer.attachProviderTransactionId.mockResolvedValue(undefined);
  ...
  it("decrements stock when the charge is approved", async () => {
    ...
    expect(writer.attachProviderTransactionId).toHaveBeenCalledWith(
      pendingTransaction().id,
      "wompi_1",
    );
  ...
  it("persists the provider id before polling a pending charge", async () => {
    gateway.createCardPayment.mockResolvedValue({
      providerTransactionId: "wompi_poll",
      status: TransactionStatus.Pending,
    });
    ...
    writer.attachProviderTransactionId.mockImplementation(async () => {
      attachOrder.push("attach");
    });
    gateway.getPaymentStatus.mockImplementation(async () => {
      attachOrder.push("poll");
      return {
        providerTransactionId: "wompi_poll",
        status: TransactionStatus.Approved,
      };
    ...
    await useCase.execute(pendingTransaction().id, "key-poll", payRequest);

    expect(attachOrder[0]).toBe("attach");
    expect(attachOrder).toContain("poll");
    expect(attachOrder.indexOf("attach")).toBeLessThan(
      attachOrder.indexOf("poll"),
    );
  });
  ...
  it("releases the claim when the provider fails before a charge exists", async () => {
    gateway.createCardPayment.mockRejectedValue(
    ...
    expect(writer.attachProviderTransactionId).not.toHaveBeenCalled();
    expect(idempotency.abort).toHaveBeenCalledWith("key-8");
  });

  it("keeps the provider id when polling fails after attach", async () => {
    gateway.createCardPayment.mockResolvedValue({
      providerTransactionId: "wompi_keep",
      status: TransactionStatus.Pending,
    });
    gateway.getPaymentStatus.mockRejectedValue(
      new PaymentGatewayError("poll down"),
    );
    ...
    expect(writer.releaseClaim).not.toHaveBeenCalled();
    expect(idempotency.abort).toHaveBeenCalledWith("key-keep");
  });
